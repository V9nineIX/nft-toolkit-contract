...
abstract contract ERC1155Factory is ERC1155, Controllable, Initializable {
...
//   event MaxSupply_Change(uint64 id, uint64 maxSupply);
  ...
  /// @notice add new item
  function addItem(uint64 maxSupply) internal {
    Item memory item = Item(uint64(items.length), maxSupply, 0, 0);
    items.push(item);
    maxMint += maxSupply;
  }
  ...
  /*
  function editItem_maxSupply(uint64 id, uint64 maxSupply) external onlyOwner {
    if (!exists(id)) revert ERC1155_Error404();
    Item storage item = items[id];

    require(maxSupply > 0, "");
    require(item.minted == 0, "");

    if (item.minted > 0 && maxSupply <= item.minted) {
        revert ERC1155_Error404();
    }
...
contract Template1155 is ERC1155Factory, Pausable {
  event Received(address, uint);

  enum ROUND { None, Private, Public }
  ...
  uint256 public privateRoundMintPrice = 0;
  uint256 public publicRoundMintPrice = 0;
...
function initialize(bytes calldata initArg1, bytes calldata initArg2, address owner) public initializer {
  ...
  /* -------------------------------------------------------------------------- */
  function addItemKeys(uint256[] memory keys, uint256[] memory amounts) private {
    require(keys.length == amounts.length, "keys and amounts length mismatch");
    for (uint i = 0; i < keys.length; i++) {
      _unusedURIKeys.push(keys[i]);
      super.addItem(uint64(amounts[i]));
    }    
  }
  ...
  /*                               Mint                                         */
  /* -------------------------------------------------------------------------- */
  function mint(uint256 amount, uint256 nonce, uint8 v, bytes32 r, bytes32 s) external payable whenNotPaused {
    // if (round == ROUND.Private) {
    //   require(isWhitelist(msg.sender), "You need to be whitelisted to mint in the private round");
    // }

    //TODO: get active state
   ...
   require(currentPhase.status ==  LibSalePhase.STATE.START , "Sale Phase not start." );
    // require(round != ROUND.None, "Mint is not yet started");
    // require(privateRoundMintPrice > 0, "Price is not yet set");
    // require(publicRoundMintPrice > 0, "Price is not yet set");
    require(amount > 0, "Quantity cannot be zero");
    require( LibSalePhase.getMintAvailableAddress(msg.sender) >= amount, "Max tokens per address exceeded for this wave");
    //require(maxMintableForTx(msg.sender)+ amount <= currentPhase.maxPerWallet, "Max tokens per address exceeded for this wave");
    //require(maxMintableForTx(msg.sender) >= amount, "You have reached maximum allowed");
    require(currentPhase.maxSupply - currentPhase.amountMinted >= amount, "Not enough key left to mint.");

   // uint256 cost = 0;
    // if (round == ROUND.Private) {
    //   cost = amount * privateRoundMintPrice;
    // } else {
    //   cost = amount * publicRoundMintPrice;
    //   require(_nonces[nonce] == 0, "Duplicated nonce");
    //   require(_recoverAddress(nonce, v, r, s) == _signer, "Invalid signature");
    //   _nonces[nonce] = 1;
    // }
   ...
   // _addressTokenMinted[msg.sender] = _addressTokenMinted[msg.sender] + amount;
  }
  ...
  function _mintRandom(address to, uint256 quantity) private {
     uint256 tokenId = takeTokenIdRandom();
  ...
  function takeTokenIdRandom() public returns (uint256) {
    ...
    keccak256(
        abi.encode(
            ...
            tx.gasprice,
            _unusedURIKeys.length
            )
        )
    );

    uint256 index = randomNum %  _unusedURIKeys.length;
    uint256 picked_key = _unusedURIKeys[index];

    items[picked_key].minted++;
    if (items[picked_key].minted >= items[picked_key].maxSupply) {
        _unusedURIKeys[index] = _unusedURIKeys[_unusedURIKeys.length - 1];
        _unusedURIKeys.pop();
    }
  ...
  function maxMintable(address addr) public view virtual returns (uint256) {
    ...
    if (round == ROUND.Private && isWhitelist(addr)) {
      max_mint = MAX_MINT_PRIVATE_ROUND;
    } else if (round == ROUND.Public) {
      max_mint = maxMintPerAddress;
    }
  ...
  function isWhitelist(address c) public view returns (bool) {
    if (_whitelist[c] > 0) {
        return true;
    } 
    return false;
  ...
  function addWhitelist(address c) public onlyOwner {
      _whitelist[c] = 1;
  }
